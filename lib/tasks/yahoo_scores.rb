...
#path = "nfl/scoreboard?w=#{ARGV[0]}"
path = "/nfl/scoreboard?w=p3"
...
Net::HTTP.get(host, path).lines do |line|
  ...
  if line =~ /^<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"1\">/
    in_box = true
    puts line
  end

  if line =~ /<b><a href="\/nfl\/teams\/([a-z]{2,3})\">(.*)<\/a><\/b>/
    data = Regexp.last_match
    team_code = data[1] 
    in_box = false
    puts line
    #puts "team_code: #{team_code}"
  end

  if line =~ /<span class=\"yspscores\">(\d*)&nbsp;<\/span>/
    data = Regexp.last_match
    score = data[1] 
    #puts "score: #{score}"
  end
